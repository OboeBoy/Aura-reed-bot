...
export default {
  ...
  execute: async (sock, message, args, { prefix }) => {
    ...
    if (!prompt) {
      ...
      text += `┃ > que DeepSeek pueda responderte.\n\n`;
    ...
    const copilotUrls = [
      `https://api.alyacore.xyz/ai/deepseek?text=${encodeURIComponent(prompt)}&key=${global.Apis.apiAiya.apikey}`,
    ];
    ...
    try {
      ...
      let resultText = `╭〔 🤖 ${fytBold("DEEP SEEK AI")} 〕⬣\n\n`;
      resultText += `${reply}\n\n`;
      ...
      console.error("Error al obtener respuesta de Deep Seek:", error);
      await sock.sendPresenceUpdate("paused", remotxeJid);
      return sock.sendMessage(
        message.key.remoteJid,
        {
